...
class Home extends BaseController
{
    public function index(): string
    {
        ...
        // Bar essentials collections with 4 preview images each
        $barCollections = [];
        foreach (['Whiskey Essentials' => 'whi-glasses', 'Cocktail Essentials' => 'cocktail-glass', 'Wine Essentials' => 'wine-glasses', 'Bar Accessories' => 'bar-accessories'] as $name => $slug) {
            ...
            if ($cat) {
                ...
                // Grab 4 product images for the grid preview
                $previewImgs = $db->table('product_media pm')
                    ->select('pm.path')
                    ->join('products p', 'p.id = pm.product_id')
                    ...
                    ->limit(4)
                    ->get()->getResultArray();
                $barCollections[] = array_merge($cat, [
                    'display_name'  => $name,
                    'count'         => $count,
                    'preview_images'=> array_column($previewImgs, 'path'),
                ]);
            }
        }
