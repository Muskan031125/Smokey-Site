...
<title>Login — Smokey Cocktail</title>
...
:root{
  ...
  --gold:#d4a849; --gold-neon:#ffd966; --copper:#c47d4a;
  --text:#f0e8d4; --muted:#9a9080; --red:#e6504a; --green:#22c55e;
}
...
input:focus{border-color:var(--gold-neon)!important;outline:none!important;
  ...
  background:rgba(20,16,16,.85)!important;}
...
.glow-text{text-shadow:0 0 20px rgba(255,217,102,.5);animation:glow 4s ease-in-out infinite;}
...
<div style="width:100%;max-width:430px;position:relative;animation:fadeUp .7s ease both;">
    <!-- Logo -->
    <div style="text-align:center;margin-bottom:2.25rem;">
        <a href="<?= site_url('/') ?>" style="text-decoration:none;">
            <div style="display:inline-flex;align-items:center;gap:.625rem;justify-content:center;">
    ...
    <div style="background:linear-gradient(180deg,rgba(28,24,18,.6),rgba(20,16,16,.85));
                ...
                box-shadow:0 25px 50px rgba(0,0,0,.5),0 0 60px rgba(212,168,73,.08),inset 0 1px 0 rgba(212,168,73,.1);">
        <h1 class="font-serif glow-text" style="font-size:1.6rem;color:var(--text);margin-bottom:.3rem;font-weight:500;">
            Welcome <em style="color:var(--gold-neon);font-style:italic;">Back</em>
        </h1>
        <p style="font-size:.825rem;color:var(--muted);margin-bottom:1.75rem;">Sign in to your account</p>
        ...
        <form method="post" action="<?= url_to('login') ?>">
            ...
            <div style="margin-bottom:1.1rem;">
                <label style="display:block;font-size:.68rem;letter-spacing:.15em;text-transform:uppercase;color:var(--muted);margin-bottom:.5rem;font-weight:600;">Email Address</label>
                <input type="email" name="email" autocomplete="email" required value="<?= esc(old('email')) ?>" placeholder="you@example.com">
            </div>
            <div style="margin-bottom:1.1rem;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
                <label style="display:flex;align-items:center;gap:.5rem;font-size:.78rem;color:var(--muted);cursor:pointer;">
                    <input type="checkbox" name="remember" style="accent-color:var(--gold-neon);width:auto!important;border:none!important;background:transparent!important;padding:0!important;"> Remember me
                </label>
                <a href="<?= url_to('magic-link') ?>" style="font-size:.78rem;color:var(--gold-neon);text-decoration:none;font-weight:600;">Forgot password?</a>
            </div>
        ...
        <div style="text-align:center;margin-top:1.5rem;padding-top:1.25rem;border-top:1px solid rgba(212,168,73,.1);">
            <p style="font-size:.8rem;color:var(--muted);">
                New to Smokey? <a href="<?= site_url('register') ?>" style="color:var(--gold-neon);text-decoration:none;font-weight:600;">Create an account →</a>
            </p>
        </div>
    </div>
